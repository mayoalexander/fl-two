	...
	*/
	...
	public function saveToBackUpDatabase($file) {
		...
		$sql= "INSERT INTO  `amrusers`.`feed` (
			...
			VALUES (
			NULL , '$file[type]', '$file[blog_story_url]', '$file[trackname]' , '$file[trackmp3]', '$file[twitter]', '$file[trackname]', '$file[user_name]', '$file[email]', '$file[phone]', '$file[submission_date]', '$file[twitpic]', '$file[photo]', '$file[playerpath]', '$file[blog_post_key]', '$file[blogentry]','$file[writeup]'
			)";
		include(ROOT.'inc/connection.php');
		if (mysqli_query($con, $sql)) {
			$status = true;
...
I. Verify User & Authentication
II. Prepare The File Contents
III. Upload Files To Main Server
IV. Upload Files to Radio Server 
I. Upload Files to Backup Server
VI. Send Notification Emails to the User & Staff
...
if ($_POST) {
	...
	// VERYIFY USER EXISTS!
	if ($upload->checkIfUserExists($_POST['user_name'])==false) {
		// STOP THE PRESSES!!!!!! THIS NIGGA DONT EXIST!!!
		echo 'This User Does Not Exist!';
	} else {
		// IF USER EXISTS, PROCEED
		//echo 'Proceed with Upload!<hr>';
		// ** Detect File Upload ** //
		if ($_POST['redirect_source']=='fl_drive_post') {
			$file = $upload->handlePhotoUpload($_POST);
			
			if ($upload->saveToBackUpDatabase($file)) {
				//$upload->saveToDatabase($file);
				//$upload->sendMail($file['email'], $file['trackname'] , $file['twitter'] , $file['trackmp3'], $file['photo'] );
				echo '<h1 class="alert alert-success" style="color:green;" ><span class="glyphicon glyphicon-ok" ></span> Upload Completed!</h1>';
			}
		} else {
					// FORMAT FILE DATA
					$file = $upload->handleSyntax($_POST);
					//$file['thumbnail'] = $upload->createthumb($file['photo']);
					if ($file['user_name']!='submission') {
						$file['twitpic'] = $upload->getTwitpicURL($file);
					}
					//print_r($file); 
					//exit;
		
					if ($upload->saveToDatabase($file)) {
						$upload->saveToBackUpDatabase($file); // backup the save
						//$radioupload = array($file['user_name'], $file['trackname'] , $file['twitter'] , $file['trackmp3'], $file['photo']);
						if ($upload->uploadToRadio($file)===false) {
							echo 'something went wrong! Not Uploaded to radio';
						} else {
							// SEND NOTIFICATIONS
							$upload->sendMail($file['email'], $file['trackname'] , $file['twitter'] , $file['trackmp3'], $file['photo'] , $file['phone']  );
							echo '<h1 class="alert alert-success" style="color:green;" ><span class="glyphicon glyphicon-ok" ></span> Upload Completed!</h1>';
