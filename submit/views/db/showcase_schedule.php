...
//echo 'username: '.$user_name_session;
...
if ($_POST['formdata']) {
				...
				//$_POST['user_name'] = '';
				$_POST['user_name'] = $_POST['formdata'][5]['value'];
				$_POST['user_email'] = $_POST['formdata'][4]['value'];
				$_POST['showcase_day'] = $_POST['formdata'][2]['value'];
				$_POST['submission_date'] = $_POST['formdata'][6]['value'];
				$_POST['type_of_event'] = $_POST['formdata'][0]['value'];
				$_POST['description'] = $_POST['formdata'][3]['value'];
				$_POST['event_title'] = $_POST['formdata'][1]['value'];
				$_POST['submit'] = true;
			}
...
$body_template = '					<head>
			...
			$stafflist = array($user_email//,
			 ...
			 //foreach ($stafflist as $admin) {
								...
								$mail->setFrom('admin@freelabel.net', 'FREELABEL BOOKING');
								//Set an alternative reply-to address
								$mail->addReplyTo('admin@freelabel.net', 'FREELABEL BOOKING');
...
'.$twitpic*/);
				$tweet_event	=	'https://twitter.com/intent/tweet?screen_name=&text='.$tweet_event;
					...
					if ($showcase_status_bool==2) {
						...
						$reschedule_event_button 	= "<form class='reschedule-event-form' style='display:inline;' method='POST' action='submit/update.php' ><input name='event_id' type='hidden' value='".$event_id."'><input name='event_action' type='hidden' value='reschedule'><input type='submit' class='btn btn-default btn-xs' value='RESCHEDULE'></form>";
					}
		...
		if ($showcase_date_primary == $next_date_bruh_2) {
				...
				if (strtotime($showcase_date_primary) < strtotime($todays_date_bruh)) {
					if ($already_posted5==0) {
										$schedule_listing .= '
										<tr>
										  	<td><h2 class="text-muted older-heading" style="color:#FE3F44;">Recent Events..</h2><p><a name="olderPosts" href="#olderPosts" class="older_post_button" onclick="showOlerEntries()">Show</a></p></td>
										  	<td></td>
											<td></td>
										    <td></td>
										    <td></td>
...
if ($row == false) {
	//$schedule_listing = '<alert>You Have No Events Booked!</alert>';
}
/* ---------------------------------------------------------------- //
...
.older , .older-heading , .event-option-add-form {
	display:none;
}
.event-option-panel {
	text-align: left;
...
<script>
 $(document).ready(function() {
     $('.edit').editable('http://freelabel.net/submit/update.php',{
     	id   : 'event_description_id',
     	//type    : 'textarea',
        cssclass  : 'editable-area',
        tooltip   : 'Click to edit event...',
        name : 'value'
     });
     $('.edit_area').editable('http://www.example.com/save.php', {
         ...
         submit    : 'Save',
         cssclass  : 'editable-area',
        // indicator : '<img src="http://freelabel.net/indicator.gif">',
         tooltip   : 'Click to edit...'
     });



     $('.add-new-event-form').on( "submit", function( event ) {
	...
	*  Reschedule Event
	---------------------------------------- */
$(function(){
	$('.reschedule-event-form').on( "submit", function( event ) {
	  event.preventDefault();
	  var formdata = $( this ).serializeArray();
	  //console.log( formdata );
	  //alert(formdata);
	  c = confirm("Are you sure you want to reschedule this event?");
	  if (c===true) {

	  	$.post( "http://freelabel.net/submit/update.php", { formdata })
		  .done(function( data ) {
		  	//alert( data );
	...
	*  Archive Event class='archive-event-form'
	...
	$('.archive-event-form').on( "submit", function( event ) {
	  	event.preventDefault();
	  	var form = $(this);
	  	var formdata = form.serializeArray();
	  	var event_id = formdata[0]['value'];
		...
		//var archiveEventForm = $('#complete_event_' + event_id);
		//var archiveEventData = $(archiveEventForm).serializeArray();
		//console.log(archiveEventForm);
		if (confirm('Are you sure you want to archive this event?')==true) {
						//alert(formdata);
						$.post('submit/update.php', { formdata }).done(function( data ) {
							if ( data != "Sucessfully Archived") {
								alert('didnt work! '+ data);
								console.log(data);
							} else {
								$('#event_row_'+ event_id +'_status').html('<label class=\"label label-success\" >Completed + Archived!</label>');
								setTimeout(function(){
									$('#event_row_' + event_id).hide('fast');
								});
							}
						});
		}
...
$('.event-option-add').click(function(){
	initial = 'Add New Event';
	//alert(initial);
	changed = 'Cancel';
	if ($(this).html()=='Cancel') {
		$(this).html(initial);
...
<nav class='event-option-panel btn-group' style='background-color:transparent;text-align:left;border-bottom:3px solid #303030;padding:2% 0%;'>
	...
	<a onclick="loadPage('http://freelabel.net/submit/views/db/showcase_schedule.php?control=booking#events', '#main_display_panel', 'performances' , '<?php echo $_SESSION["user_name"]; ?>')" href='#' class='btn btn-default btn-xs'>Performances</a>
	<a onclick="loadPage('http://freelabel.net/submit/views/db/showcase_schedule.php?control=booking#events', '#main_display_panel', 'clients' , '<?php echo $_SESSION["user_name"]; ?>')" href='#' class='btn btn-default btn-xs'>Public</a>
	<!--<button class='btn btn-default btn-xs event-option-add'><span class='glyphicon glyphicon-plus'></span> Add New Event</button>-->
</nav>

<div class="panel panel-default event-option-add-form">
			  <!-- SUBMIT FORM -->
