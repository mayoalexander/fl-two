...
class LoginModel
{
    ...
    {
        if (isset($_POST["captcha"]) AND (true)) {
            return true;
        }
        // default return
        return false;
    ...
    public function registerWithFacebook()
    {
        ...
        if ($user) {
            try {
                // Proceed knowing you have a logged in user who's authenticated
                $facebook_user_data = $facebook->api('/me?fields=email,name,picture,link,website');
                // echo '<pre>';
        ...
        if (!$facebook_user_data) {
            ...
            //$_SESSION["feedback_negative"][] = '<script>window.location.assign("'.$this->getFacebookLoginUrl().'")</script>';
            return false;
        }
    ...
    public function registerNewUserWithFacebook($facebook_user_data)
    {
        ...
        $repl = array('.',' ');
        $clean_user_name_from_facebook = str_replace($repl, "", $facebook_user_data["name"]);
        // generate integer-timestamp for saving of account-creating date
        $user_creation_timestamp = time();
